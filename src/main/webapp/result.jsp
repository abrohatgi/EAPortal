<%-- 
    Document   : result
    Created on : Feb 23, 2015, 2:49:14 PM
...
<%
    if (session.getAttribute("ProjectID") != null) {
        ...
        System.out.println("The task is :" + request.getAttribute("task"));
        ...
        if (request.getAttribute("fileName") != null) {
            ...
            EvalArray = ref.readExcelFile(session.getAttribute("ProjectID").toString(), fileName);

            EmailAnalysis.sendMail(EvalArray);
            String jsonStr = JsonWriter.objectToJson(EvalArray);
            String DateStr = null;
            try {
            ...
            QueryDB.insertEvalData(null, session.getAttribute("ProjectID").toString(), null, DateStr, jsonStr);
...
<%
    ...
    ReadExcelFile ref = new ReadExcelFile();
    HashMap<String, ArrayList<HashMap<String, String>>> EvalMap = ref.evaluateFromRatingDB(session.getAttribute("ProjectID").toString());
    if (EvalMap.isEmpty()) {
%>

<jsp:forward page="EAResult.jsp?status=noRatedQuestionFound"/>
            ...
            <table class="table">
                ...
                <%                        int Total = 0;
                    ...
                    while (it.hasNext()) {
                        ...
                        int responsesFound = 0;
                         ...
                         {
                           totalResponses = Integer.parseInt(responseArray.get(i).get("numResponses"));
                           goldResponses =  Integer.parseInt(responseArray.get(i).get("GoldResponses"));  
                           if (responseArray.get(i).get("Found").contains("YES")) {
                         ...
                         System.out.println("****************************************");
                         System.out.println("The total responses are: " + totalResponses );
                         System.out.println("The Gold Responses are :" + goldResponses);
                         System.out.println("The responses found are : " + responsesFound); 
                         System.out.println("****************************************");
                         
                         
                         float precision = (float)responsesFound/totalResponses;
                         float recall = (float)responsesFound/goldResponses;
                        ...
                        for (int i = 0; i < responseArray.size(); i++) {
                            ...
                            if (responseArray.get(i).get("Rank").equalsIgnoreCase("-1")) {
                                PassageRank = "-";
                            } else {
                                int Rank = Integer.parseInt(responseArray.get(i).get("Rank").toString()) + 1;
                                PassageRank = Integer.toString(Rank);
                                if (highestRank < Rank) {
                                    highestRank = Rank;
                                }
                            ...
                            if (responseArray.get(i).get("PassageLength").equalsIgnoreCase("-1")) {
                                PassageLength = "-";
                            } else {
                                PassageLength = responseArray.get(i).get("PassageLength");
                            }
                ...
                %>
                    ...
                    <tr>
                        ...
                        %>
                        <td align="left"><%=j%></td>
                        <td align="left"><%=responseArray.get(i).get("Query")%></td>
                        ...
                        %>
                        <td align="left"><%=responseArray.get(i).get("ExpectedResult")%></td>
                        <td align="center"><%=responseArray.get(i).get("UserRating")%></td>
                        <td align="center"><%=responseArray.get(i).get("Found")%></td>
                        <td align="center"><%=PassageRank%></td>
                        <td align="center"><%=PassageLength%></td>
                    </tr>
                    <%                        }
